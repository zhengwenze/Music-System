  ...
  <div class="user-management-container">
    <el-card>
      ...
      <div class="search-box">
        <el-input v-model="searchKeyword" placeholder="请输入用户名进行搜索" style="width: 300px; margin-right: 10px;" clearable
    ...
    const fetchUserList = async () => {
      ...
      try {
        ...
        if (response.data.code === 20 || response.data.success === true) {
          ...
          // 只显示角色为用户(role为2)的数据，添加容错处理
          const filteredUsers = allUsers.filter(user => user && user.role === 2);
